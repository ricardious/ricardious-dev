...
import { Canvas, extend, events as r3fEvents, useFrame } from '@react-three/fiber';
...
function ShaderPlane({ index, texture, project, isInView }: ShaderPlaneProps) {
	...
	const [media, setMedia] = useState<'mobile' | 'tablet' | 'desktop'>('desktop');
	...
	const clickHandler = () => {
		...
		scrollArea.current.scrollTop =
			index *
			((scrollArea.current.children[0].clientHeight - window.innerHeight) / maxProjectIndex);
		...
		>
			...
			>
				<div className="flex items-center flex-col flex-nowrap">
					<h1 className="text-h1 serif uppercase whitespace-nowrap text-white">{project.name}</h1>
					<div
					...
					>
						...
						<div className="ml-2.5 w-5 h-5 rounded-full bg-white flex items-center justify-center">
							<Icon name="arrow-right" className="w-[60%] h-[60%] opacity-60 text-black" />
						</div>
					</div>
				</div>
			</Html>
...
function Scene() {
	...
	useFrame(({ camera }, delta) => {
		...
		const scrollHeight = scrollArea.current.children[0].clientHeight - window.innerHeight;
		if (scrollHeight <= 0) return;

		scrollValue = mapRange(0, 1, 0, maxProjectIndex, scrollArea.current.scrollTop / scrollHeight);
